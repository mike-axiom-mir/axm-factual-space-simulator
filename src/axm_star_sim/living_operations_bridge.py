...
OPERATIONS_VERSION = "0.3.0-candidate"
...
def build_operations_context(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    for thread_id in open_ids[:8]:
        ...
        thread_rows.append(
            {
                "thread_id": thread_id,
                "title": raw.get("title", thread_id),
                "kind": raw.get("kind", "unknown"),
                "status": raw.get("status", "unknown"),
                "depth": raw.get("depth"),
                "visits": raw.get("visits"),
                "target_planet_id": raw.get("target_planet_id"),
                "updated_turn": raw.get("updated_turn"),
                "authority": "read_only_runtime_summary",
            }
        )
        ...
        action_rows.append(
            {
                ...
                "source_thread": raw.get("source_thread"),
                "target_planet_id": raw.get("target_planet_id"),
                "intent": raw.get("intent"),
                "priority": raw.get("priority"),
                "authority": "inspect_only_not_executable",
                "may_execute": False,
            }
        )
    ...
    packet = {
        ...
        "source_runtime_schema": runtime.get("schema"),
        "source_turn": runtime.get("turn"),
        "source_mission_time_hours": runtime.get("mission_time_hours"),
        "resources": resources,
        "open_thread_count": len(open_ids),
        "open_threads": thread_rows,
        "action_menu_version": action_menu.get("menu_version"),
        "available_action_count": len(actions),
        "actions": action_rows,
        "authority": "read_only_presentation_summary",
...
def enrich_storyboard(storyboard: dict[str, Any], runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    out["operations_context"] = build_operations_context(runtime)
    out["living_operations"] = {
        ...
        "action_mode": "inspect_only_not_executable",
    ...
    out["living_operations"]["profile_hash"] = _hash(
        out["living_operations"], "AXM-LIVING-OPERATIONS-PRESENTATION-PROFILE-V1"
    )
    return out
